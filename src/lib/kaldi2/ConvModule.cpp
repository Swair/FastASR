...
void ConvModule::forward_mode0(Tensor<float> *din)
{
    ...
    Tensor<float> conv_in(1, mm + 30);
    Tensor<float> blasin(mm, 31);
    conv_in.zeros();

    for (i = 0; i < 512; i++) {
        for (j = 0; j < mm; j++) {
            int ii = j * 512 + i;
            conv_in.buff[j + 15] = din->buff[ii];
            din->buff[ii] = params->depthwise_conv_bias[i];
        }
        for (j = 0; j < mm; j++) {
            int offset = j * 31;
            memcpy(blasin.buff + offset, conv_in.buff + j, 31 * sizeof(float));
        }

        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, mm, 1, 31, 1,
                    blasin.buff, 31, params->depthwise_conv_weight + i * 31, 1,
                    1, din->buff + i, 512);
    }
...
void ConvModule::forward_mode1(Tensor<float> *din)
{
    ...
    //     for (j = 0; j < mm; j++) {
    //         int offset = j * 15;
    //         memcpy(blasin.buff + offset, conv_in.buff + j, 15 *
    //         sizeof(float));
    //     }

    //     cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, mm, 1, 15, 1,
    //                 blasin.buff, 15, params->depthwise_conv_weight + i * 15,
    //                 1, 1, din->buff + i, 512);
    // }
    // int t_offset = (mm - 14) * 512;
    // memcpy(conv_cache->buff, tmp_din.buff + t_offset, 14 * 512 *
    // sizeof(float));
